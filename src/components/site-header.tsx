  ...
  return (
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-purple-100 relative">
      <div className="max-w-6xl mx-auto px-5 sm:px-8 h-16 sm:h-20 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-7 text-sm whitespace-nowrap font-medium text-ink-700">
          {NAV_ITEMS.map((item) => (
            <Link key={item.href} href={item.href} className="py-2 hover:text-purple-600 transition-colors">
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <Link
          ...
          className="hidden lg:inline-flex items-center whitespace-nowrap rounded-full bg-purple-500 hover:bg-purple-600 text-white px-5 py-2.5 text-sm font-semibold shadow-md shadow-purple-300/40 hover:shadow-lg hover:shadow-purple-400/40 transition-all duration-200 hover:-translate-y-0.5"
        >
          Agendar consulta
        </Link>
